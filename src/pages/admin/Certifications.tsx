...
import {
    ...
    XCircle, RefreshCcwIcon, BadgeCheck
} from 'lucide-react';
...
import {UseAddCertification, UseDeleteCertification,
    UseGetCertifications,
    UseUpdateCertification} from "../../services";
...
const CertificationsPage: React.FC = () => {
    ...
    const {isPending: isGettingPartners, isRefetching: isReGettingPartners, data: partners, refetch: reGetPartners} = UseGetCertifications({format: "stats"})
    const {data: addResult, isPending: isAdding, mutate: addPartner} = UseAddCertification()
    const {data: updateResult, isPending: isUpdating, mutate: updatePartner} = UseUpdateCertification()
    const {data: deleteResult, isPending: isDeleting, mutate: deletePartner} = UseDeleteCertification()
    ...
    useEffect(() => {
        if (addResult) {
            if (addResult?.responseData?.error) {
                ...
                reGetPartners()
                AppToast.success(theme === "dark", 'Certification ajoutée avec succès')
    ...
    useEffect(() => {
        if (updateResult) {
            if (updateResult?.responseData?.error) {
                ...
                reGetPartners()
                AppToast.success(theme === "dark", 'Certification mise a jour avec succès')
                setShowFormModal(null);
                setFormData(emptyItem);
    ...
    useEffect(() => {
        if (deleteResult) {
            if (deleteResult?.responseData?.error) {
                ...
                reGetPartners()
                AppToast.success(theme === "dark", 'Certification supprimée avec succès')
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.title.trim()) {
            AppToast.error(isDark, 'Le nom est requis');
            return;
        }
        const body = {
    ...
    return (
        <div>
            <AdminPageHeader
                ...
                onAdd={HasPermission(appPermissions.certifications, appOps.create) ? () => {
                    setFormData(emptyItem);
                    ...
                    {Object.entries(grouped).map(([groupName, groupPartners]) => (
                        ...
                        >
                            ...
                            {isActive(partner) ? 'Actif' : 'Inactif'}
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div className="flex items-center justify-end gap-2">
                                                {HasPermission(appPermissions.certifications, appOps.update) ? <button
                            ...
                            />
                                                ...
                                                {/* Supprimer */}
                                                {HasPermission(appPermissions.certifications, appOps.delete) ? <button
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                {showFormModal === "edit" ? 'Modifier la Certification' : 'Ajouter une Certification'}
                            </h2>
                            <button
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    ...
                    >
                        <div className="flex items-center mb-4">
                            ...
                            >
                                {isActive(showStatusConfirm) ? "Desactiver" : "Approuver"}
                            </h3>
                        </div>
                        <p
                            className={`mb-6 ${
                        ...
                        >
                            Voulez-vous vraiment changer le statut cette certification ? :
                            {showStatusConfirm && (
