...
import {
  Users, Eye, UserCheck, UserX, Calendar, Plus, Edit, Trash2,
...
const isActive = (u: User) =>
  u.profile && typeof u.profile.is_active === 'boolean'
    ? u.profile.is_active
...
const UsersPage: React.FC = () => {
  ...
  const fetchCurrentUserPermissions = async () => {
    try {
      ...
      if (allRoles.length === 0) { setCanResetPasswords(false); return; }
      ...
      const isAdminLike = aggregated.can_delete || aggregated.can_add || aggregated.can_reset_password;
      setIsCurrentAdmin(isAdminLike);
  ...
  const handleRefresh = async () => {
    ...
    await loadCustomRoles();
    setRefreshing(false);
    Swal.fire({ icon: 'success', title: 'Succès', text: 'Données actualisées' });
  };
  ...
  const handleConfirmReset = async (targets: User[]) => {
    if (!targets || targets.length === 0) {
      Swal.fire({ icon: 'error', title: 'Erreur', text: 'Aucun utilisateur sélectionné pour la réinitialisation.' });
      return;
    }
  ...
  const handleUpdateUserStatus = async (user: User) => {
    ...
    try {
      ...
      Swal.fire({ icon: 'error', title: 'Erreur', text: error.message || 'Erreur lors de la mise à jour du statut' });
  ...
  const getActionItems = (user: User) => [
    ...
    },
    {
      ...
      icon: KeyRound,
      onClick: () => { if (canResetPasswords) setShowResetConfirm(user); },
  ...
  const groupedUsers = () => {
    ...
    return filteredUsers.reduce((acc, user) => {
      ...
      if (groupBy === 'status') {
        key = isActive(user) ? 'Actifs' : 'En attente';
      } else if (groupBy === 'role') {
        if (user.isAdmin) key = 'Administrateurs';
        else if (user.isPartner) key = 'Partenaires';
        else if (user.customRoles && user.customRoles.length > 0) key = 'Rôles personnalisés';
        else key = 'Utilisateurs';
  ...
  const getRoleDistribution = () => {
    ...
    users.forEach((u) => {
      const roleName = u.mainRoleName && u.mainRoleName.trim().length > 0
        ? u.mainRoleName
        : 'Sans rôle';
  ...
  return (
    <div>
      {isResetRunning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className={
            ...
            <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_top,_rgba(34,197,94,0.35),_transparent_55%),radial-gradient(circle_at_bottom,_rgba(56,189,248,0.2),_transparent_55%)] opacity-70" />
            ...
            <div className="mb-2 h-2 w-full overflow-hidden rounded-full border border-emerald-400/40 bg-slate-900/80 shadow-[0_0_18px_rgba(34,197,94,0.6)]">
              <div
            ...
            <div className={theme === 'dark' ? 'flex items-center justify-between text-[11px] font-mono text-gray-300' : 'flex items-center justify-between text-[11px] font-mono text-gray-700'}>
              <span className={theme === 'dark' ? 'uppercase tracking-wide text-emerald-300/80' : 'uppercase tracking-wide text-emerald-700/80'}>Mode sécurisé Matrix</span>
      ...
      <div className={`flex items-center mt-10 justify-between mb-6 py-3 px-2 ${
        theme === 'dark' ? 'bg-gray-900' : 'bg-white border-b border-gray-200'
      }`}>
        <h1 className={`text-2xl font-bold flex items-center gap-2 flex-1 min-w-0 whitespace-nowrap overflow-hidden text-ellipsis ${
          theme === 'dark' ? 'text-white' : 'text-gray-900'
        }`}>
          <Users className={`w-6 h-6 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`} />
          {currentView === 'visitors' ? 'Gestion des Visiteurs' :
        ...
        <div className="flex items-center space-x-3 flex-shrink-0 px-2">
          <button
            ...
            disabled={refreshing}
            className={`btn px-4 py-2 rounded-lg flex items-center transition-colors disabled:opacity-50 ${
              theme === 'dark'
                ? 'bg-gray-700 hover:bg-gray-600 text-white'
                : 'bg-white border border-gray-300 text-gray-800 hover:bg-slate-50'
            }`}
          >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h2 className={theme === 'dark' ? 'text-xl font-semibold text-white' : 'text-xl font-semibold text-gray-900'}>Ajouter un utilisateur</h2>
              <button
            ...
            <form onSubmit={handleAddUser} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className={theme === 'dark' ? 'block text-sm text-gray-300 mb-1' : 'block text-sm text-gray-700 mb-1'}>Téléphone</label>
                  <input
                    type="text"
                    value={formData.phone}
                    onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
      ...
      <div>
                      ...
                      <tbody className={theme === 'dark' ? 'bg-gray-900 divide-y divide-gray-700' : 'bg-white divide-y divide-gray-200'}>
                        {groupUsers.map((user) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-sm">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  >
                                    {user.profile?.full_name || user.user_metadata?.name || user.email}
                                  </div>
                                  <div
                                    className={`text-xs ${
                            ...
                            >
                              {(() => {
                                const label = (user.mainRoleName || '').trim() || 'user';
                                const slug = label
                                  .toLowerCase()
                                  .replace(/[^a-z0-9]+/g, '-')
                                  .replace(/(^-|-$)/g, '');

                                if (slug === 'super-admin' || slug === 'super-administrateur') {
                                  return (
                                    <span className="inline-flex items-center rounded-md border border-fuchsia-500/70 px-2.5 py-1 text-xs font-medium text-white bg-fuchsia-600">
                                      {label}
                                    </span>
                                  );
                                }

                                if (slug === 'admin-junior' || slug === 'admin-jr') {
                                  return (
                                    <span className="inline-flex items-center rounded-md border border-indigo-500/70 px-2.5 py-1 text-xs font-medium text-white bg-indigo-600">
                                      {label}
                                    </span>
                                  );
                                }

                                if (slug.includes('gestionnaire') || slug.includes('gestionnaires-de-roles') || slug.includes('gestionnaires-des-roles')) {
                                  return (
                                    <span className="inline-flex items-center rounded-md border border-emerald-500/70 px-2.5 py-1 text-xs font-medium text-white bg-emerald-600">
                                      {label}
                                    </span>
                                  );
                                }
                                ...
                                if (user.isAdmin) {
                                  return (
                                    <span className="inline-flex items-center rounded-md border border-purple-500/60 px-2.5 py-1 text-xs font-medium text-white bg-purple-600">
                                      {label}
                                    </span>
                                  );
                                }
                                ...
                                if (user.isPartner) {
                                  return (
                                    <span className="inline-flex items-center rounded-md border border-amber-500/60 px-2.5 py-1 text-xs font-medium text-white bg-amber-500">
                                      {label}
                                    </span>
                                  );
                                }
                                ...
                                return (
                                  <span className="inline-flex items-center rounded-md border border-sky-500/60 px-2.5 py-1 text-xs font-medium text-white bg-sky-500">
                                    {label}
                                  </span>
                                );
                              })()}
                            ...
                            <td className="px-4 py-3 text-sm text-right">
                              <div className="inline-flex w-full flex-wrap items-center justify-end gap-2">
                                {/* Toggle statut en premier */}
                                {getActionItems(user)
                                  .filter((action) => action.label === 'Désactiver compte' || action.label === 'Activer compte')
                                  .map((action) => (
                                    <button
                                      key={action.label}
                                      type="button"
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div className={theme === 'dark' ? 'bg-gray-700/50 rounded-lg p-3' : 'bg-gray-50 rounded-lg p-3'}>
                  <div className={theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}>Statut</div>
                  <div className={theme === 'dark' ? 'text-white font-medium' : 'text-gray-900 font-medium'}>
                    {showViewModal.email_confirmed_at ? 'Actif' : 'Inactif'}
                  </div>
                </div>
                <div className={theme === 'dark' ? 'bg-gray-700/50 rounded-lg p-3' : 'bg-gray-50 rounded-lg p-3'}>
                  <div className={theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}>Date de création</div>
                  <div className={theme === 'dark' ? 'text-white font-medium' : 'text-gray-900 font-medium'}>
                    {showViewModal.created_at ? format(parseISO(showViewModal.created_at), 'dd/MM/yyyy HH:mm', { locale: fr }) : '—'}
                  </div>
                </div>
                <div className={theme === 'dark' ? 'bg-gray-700/50 rounded-lg p-3' : 'bg-gray-50 rounded-lg p-3'}>
                  <div className={theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}>Dernière connexion</div>
                  <div className={theme === 'dark' ? 'text-white font-medium' : 'text-gray-900 font-medium'}>
                ...
                <div className={theme === 'dark' ? 'bg-gray-700/50 rounded-lg p-3' : 'bg-gray-50 rounded-lg p-3'}>
                  ...
                  <div className={theme === 'dark' ? 'text-white font-medium break-words' : 'text-gray-900 font-medium break-words'}>
                    {showViewModal.roles && showViewModal.roles.length > 0 ? showViewModal.roles.join(', ') : '—'}
                  </div>
                </div>
                {showViewModal.profile && (
                  <div className={theme === 'dark' ? 'md:col-span-2 bg-gray-700/50 rounded-lg p-3' : 'md:col-span-2 bg-gray-50 rounded-lg p-3'}>
                    <div className={theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}>Société</div>
                    <div className={theme === 'dark' ? 'text-white font-medium' : 'text-gray-900 font-medium'}>{showViewModal.profile.company || '—'}</div>
                    <div className={theme === 'dark' ? 'text-gray-400 mt-2' : 'text-gray-500 mt-2'}>Téléphone</div>
                    <div className={theme === 'dark' ? 'text-white font-medium' : 'text-gray-900 font-medium'}>{showViewModal.profile.phone_number || '—'}</div>
                  </div>
                )}
              </div>
        ...
        {showEditModal && selectedUser && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            ...
            >
              <div className="flex justify-between items-center mb-4">
                <h2 className={theme === 'dark' ? 'text-xl font-semibold text-white' : 'text-xl font-semibold text-gray-900'}>Modifier l'utilisateur</h2>
                <button
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            ...
            >
              <div className="flex items-center mb-4">
                ...
                <h2 className={theme === 'dark' ? 'text-xl font-semibold text-white' : 'text-xl font-semibold text-gray-900'}>Confirmer la réinitialisation</h2>
              </div>
              <p className={theme === 'dark' ? 'text-gray-300 mb-6' : 'text-gray-700 mb-6'}>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h2 className={theme === 'dark' ? 'text-xl font-semibold text-white' : 'text-xl font-semibold text-gray-900'}>Attribuer un rôle</h2>
                <button
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {customRoles.length > 0 ? (
                        customRoles.map((customRole) => (
                          <label key={customRole.id} className={theme === 'dark' ? 'flex items-center p-3 bg-gray-700 rounded-lg hover:bg-gray-600 cursor-pointer' : 'flex items-center p-3 bg-gray-100 rounded-lg hover:bg-gray-200 cursor-pointer'}>
                            <input
                              type="radio"
                              name="role"
                              ...
                              className="mr-3"
                            />
                            <div className={theme === 'dark' ? 'flex items-center text-white' : 'flex items-center text-gray-900'}>
                              <Crown className="w-4 h-4 mr-2 text-purple-400" />
                              <div>
                                <div className="font-medium">{customRole.name}</div>
                                {customRole.description && (
                                  <div className={theme === 'dark' ? 'text-xs text-gray-400' : 'text-xs text-gray-500'}>{customRole.description}</div>
                                )}
                              </div>
                            </div>
                          </label>
                        ...
                        <div className={theme === 'dark' ? 'text-center py-4 text-gray-400' : 'text-center py-4 text-gray-500'}>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            ...
            >
              <div className="flex items-center mb-4">
                ...
                <h2 className={theme === 'dark' ? 'text-xl font-semibold text-white' : 'text-xl font-semibold text-gray-900'}>Supprimer l'utilisateur</h2>
              </div>
              <p className={theme === 'dark' ? 'text-gray-300 mb-6' : 'text-gray-700 mb-6'}>
