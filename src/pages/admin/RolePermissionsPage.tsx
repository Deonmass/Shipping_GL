...
const RolePermissionsPage: React.FC = () => {
  ...
  const handleAddRole = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (customRoles.some(r => r.slug === slug)) {
        Swal.fire({ icon: 'error', title: 'Erreur', text: 'Ce rôle existe déjà' });
        return;
      }
      ...
      Swal.fire({
        icon: 'error',
        title: 'Erreur',
        text: error.message || 'Erreur lors de la création du rôle',
        background: isDark ? '#020617' : '#ffffff',
        color: isDark ? '#e5e7eb' : '#111827',
                  ...
                  <div className="flex items-center justify-between px-3 py-2">
                    ...
                    <div className="relative flex items-center ml-2">
                      <button
                        type="button"
                        onClick={() =>
                          setActionMenuRoleId(prev => (prev === roleSlug ? null : roleSlug))
                        }
                          ...
                          <button
                            ...
                            onClick={() => {
                              setPinnedSlugs(prev => {
                                ...
                                if (n.has(roleSlug)) n.delete(roleSlug); else n.add(roleSlug);
                                return n;
                              });
            ...
            <table className="w-full">
              <thead>
                <tr>
                  ...
                  {['Voir', 'Ajouter', 'Modifier', 'Supprimer', 'Réinitialiser MDP', 'Activer / désactiver compte'].map(
                    (label) => (
                      <th
                        key={label}
                ...
                {resources.map(resource => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 text-center">
                      <input
                        type="checkbox"
                        checked={resource === 'users' && getPermission(selectedRole, resource, 'can_reset_password')}
                        onChange={(e) =>
                          resource === 'users' &&
                          handlePermissionChange(
                            selectedRole,
                        ...
                        className={`w-5 h-5 cursor-pointer rounded-sm border-2 text-emerald-500 accent-emerald-500 focus:ring-emerald-500 focus:ring-offset-1 disabled:opacity-40 ${
                    ...
                    <td className="px-4 py-4 text-center">
                      <input
                        type="checkbox"
                        checked={resource === 'users' && getPermission(selectedRole, resource, 'can_toggle_status')}
                        onChange={(e) =>
                          resource === 'users' &&
                          handlePermissionChange(
                            selectedRole,
      ...
      {showEditRoleModal && (() => {
        ...
        const onSubmit = async (e: React.FormEvent) => {
          ...
          try {
            ...
            if (!newName) { Swal.fire({ icon: 'error', title: 'Erreur', text: 'Le nom du rôle est requis' }); return; }
            ...
            Swal.fire({ icon: 'error', title: 'Erreur', text: e?.message || 'Erreur lors de la modification' });
          }
        };
        return (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          ...
          >
            ...
            <p className={`mb-6 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
              Êtes-vous sûr de vouloir supprimer ce rôle ? Toutes les permissions associées seront également supprimées.
