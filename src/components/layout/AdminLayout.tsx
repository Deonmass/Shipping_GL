...
const AdminLayout: React.FC = () => {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (authUser?.id) {
        ...
        setCurrentUserName(nameFromProfile || email.split('@')[0] || 'Utilisateur');
      }
    });
  }, []);
  ...
  useEffect(() => {
    const loadUserMenuAccess = async () => {
      try {
        ...
        if (error) {
          console.warn('[AdminLayout] Error loading user_menu_access:', error);
          setMenuPermissionsActive(false);
          setAllowedMenuKeys(new Set());
          return;
        }
        ...
        console.error('[AdminLayout] Unexpected error loading user_menu_access:', e);
        setMenuPermissionsActive(false);
        setAllowedMenuKeys(new Set());
      } finally {
  ...
  const searchItems = [
    { key: 'dashboard', label: 'Dashboard', path: '/admin/dashboard', keywords: ['accueil', 'home'] },
    { key: 'users', label: 'Utilisateurs', path: '/admin/users', keywords: ['users', 'comptes'] },
    { key: 'users_assign_roles', label: 'Attribuer rôle', path: '/admin/users/assign-roles', keywords: ['rôles', 'permissions'] },
    { key: 'users_permissions', label: 'Permissions des rôles', path: '/admin/users/permissions', keywords: ['rôles', 'droits'] },
    { key: 'reports', label: 'Rapports', path: '/admin/reports', keywords: ['rapports', 'statistiques'] },
    { key: 'settings', label: 'Paramètres', path: '/admin/settings', keywords: ['paramètres', 'configuration'] },
    { key: 'notifications', label: 'Notifications', path: '/admin/notifications', keywords: ['notifications'] },
    // Items du bloc "tables"
    ...tableItems,
  ];

  const effectiveSearchItems = searchItems.filter((item) => !item.key || canSeeMenuKey(item.key));
  ...
  const userItems = [
    { key: 'users', path: '/admin/users', icon: Users, label: 'Utilisateurs' },
    { key: 'users_assign_roles', path: '/admin/users/assign-roles', icon: Shield, label: 'Attribuer rôle' },
    { key: 'users_permissions', path: '/admin/users/permissions', icon: Settings, label: 'Permissions des rôles' }
  ];
  ...
  return (
    <div className={`min-h-screen flex ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-slate-50 text-gray-900'}`}>
      {/* Mobile overlay */}
      <div
      ...
      >
        <div className="h-full px-3 py-4 flex flex-col">
          ...
          <ul className="space-y-2 flex-1 overflow-y-auto">
            ...
            {!menuLoading && canSeeMenuKey('dashboard') && (
            ...
            {/* Users group */}
            {!menuLoading && userItems.filter(item => canSeeMenuKey(item.key)).length > 0 && (
              <li className="relative">
                <button
                ...
                >
                  <div className="flex items-center">
                    ...
                    {sidebarExpanded && (
                      <span className="ml-3 whitespace-nowrap hidden md:inline">Utilisateurs</span>
                    )}
                  </div>
                  {sidebarExpanded && (
                ...
                {usersExpanded && (
                  <ul className="mt-2 space-y-1">
                    {userItems
                      .filter(item => canSeeMenuKey(item.key))
                      .map((item) => (
                        <li key={item.path} className="relative">
                          ...
                          >
                            ...
                            {sidebarExpanded && (
                              <span className="ml-3 whitespace-nowrap hidden md:inline">{item.label}</span>
                            )}
                          </Link>
                        </li>
                      ))}
            ...
            {/* Tables group */}
            {!menuLoading && tableItems.filter(item => canSeeMenuKey(item.key)).length > 0 && (
                        ...
                        return (
                          <li key={item.path} className="relative">
                            ...
                            >
                              <item.icon className="w-4 h-4" />
                              {sidebarExpanded && (
                                <span className="ml-3 whitespace-nowrap flex-1 flex items-center justify-between">
                                  <span>{item.label}</span>
                                  {isQuoteRequests && quoteBadge > 0 && (
                                    <span className="ml-2 inline-flex items-center justify-center min-w-[1.4rem] px-1.5 py-0.5 rounded-full text-[11px] font-semibold bg-red-600 text-white">
            ...
            {/* Notifications group */}
            {!menuLoading && notificationItems.filter(item => canSeeMenuKey(item.key)).length > 0 && (
              <li className="relative mt-2">
                <button
                ...
                >
                  <div className="flex items-center">
                    ...
                    {sidebarExpanded && (
                      <span className="ml-3 whitespace-nowrap hidden md:inline">Notifications</span>
                    )}
                  </div>
                  {sidebarExpanded && (
                    <span className="hidden md:inline">
                      ...
                      .map((item) => {
                        ...
                        return (
                          <li key={item.key} className="relative">
                            <Link
                              to={{ pathname: '/admin/notifications', search: `?type=${item.type}` }}
                              className={`flex items-center ${
                                sidebarExpanded ? 'justify-start pl-6' : 'justify-center'
                              } p-2 rounded-lg text-sm ${
                                isActive
            ...
            {!menuLoading && canSeeMenuKey('reports') && (
              <li className="relative">
                <Link
                  to="/admin/reports"
                  className={`flex items-center ${
            ...
            {!menuLoading && canSeeMenuKey('settings') && (
              <li className="relative">
                <Link
                  to="/admin/settings"
                  className={`flex items-center ${
            ...
            {!menuLoading && canSeeMenuKey('updates') && (
              <li className="relative">
                <Link
                ...
                >
                  <div className="relative flex items-center">
                    ...
                    {notificationCountsByType.update > 0 && (
                      <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] rounded-full min-w-[1.1rem] px-1 flex items-center justify-center">
            ...
            <div className="flex items-center gap-0">
              ...
              <div className="relative ml-3">
                <button
                  ...
                  title={currentUserName || currentUserEmail || 'Profil'}
                >
                  <User className={`w-5 h-5 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-600'}`} />
                </button>
                ...
                >
                  <div className={`px-4 py-3 border-b ${theme === 'dark' ? 'border-gray-700/50' : 'border-gray-200'}`}>
