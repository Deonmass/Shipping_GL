...
import {UseOpenAddNewsletter} from "../../services";
...
const RightSidebar: React.FC<RightSidebarProps> = ({events}) => {
    ...
    const {isPending: isAddingNewsletter, mutate: addNewsletter, data: addNewsletteResult} = UseOpenAddNewsletter()
    ...
    useEffect(() => {
        if (addNewsletteResult) {
            if (addNewsletteResult?.responseData?.error) {
                toast.error(addNewsletteResult?.responseData?.message || 'Une erreur est survenue lors de l\'abonnement');
            }else{
            ...
            <div className="sticky top-16 space-y-4">
                        ...
                        {events.slice(0, 4).map((event) => (
                            ...
                            >
                                <div className="flex-shrink-0 flex items-start">
                                    <div
                                        className="bg-primary-50 rounded-lg flex flex-col items-center justify-center p-2 min-w-[56px]">
                ...
                <div className="bg-white rounded-lg shadow-sm p-4">
                    ...
                    <form onSubmit={handleNewsletterSubmit} className="space-y-3">
                        ...
                        >
                            {isAddingNewsletter ? "Chargement ..." : t('news.newsletter.subscribe')}
                        </button>
                    </form>
                </div>
                            ...
                            <div className="relative p-6">
                                ...
                                <div className="flex items-start space-x-4 mb-6">
                                    <div className="flex-shrink-0">
                                        <div
                                            className="w-16 h-16 bg-primary-50 rounded-lg flex flex-col items-center justify-center">
                    <span className="text-xs font-medium text-primary-600">
                            ...
                            <div className="p-6 space-y-3">
                                {events.map((event) => (
                                    ...
                                    >
                                        <div className="flex-shrink-0">
                                            <div
                                                className="w-14 h-14 bg-primary-50 rounded-lg flex flex-col items-center justify-center">
                      <span className="text-xs font-medium text-primary-600">
                        {formatEventDate(event.event_date).split(' ')[1].toUpperCase()}
                      </span>
